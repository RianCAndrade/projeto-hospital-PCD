...
import { Stethoscope, PlayCircle, CheckCircle2, ClipboardList, Clock, Users, FileText } from "lucide-react"
...
export default function MedicoPage() {
  ...
  const { usuarioLogado, agendamentos, medicos, alterarStatus } = useHospital()
  ...
  // Encontrar o medico correspondente pelo CRM (medicos do mock)
  const meuMedico = useMemo(
    () => medicos.find((m) => m.crm === usuarioLogado?.crm),
    [medicos, usuarioLogado],
  )

  const meusAgendamentos = useMemo(
    () => agendamentos.filter((a) => a.medicoId === meuMedico?.id),
  ...
  const ematendimento = useMemo(
    () => meusAgendamentos.filter((a) => a.status === "em_atendimento"),
    [meusAgendamentos],
  )
  ...
  function iniciarAtendimento(a: Agendamento) {
    alterarStatus(a.id, "em_atendimento")
    toast.success("Atendimento iniciado", {
      description: `Status alterado para "em atendimento" para ${a.criancaNome}.`,
    })
  }
  ...
  function confirmarEncerrar() {
    ...
    alterarStatus(agSelecionado.id, "encerrado")
    toast.success("Atendimento encerrado", {
      description: `Status alterado para "atendimento encerrado" para ${agSelecionado.criancaNome}.`,
    })
  ...
  return (
    <div className="min-h-screen bg-background">
      <DashboardHeader
        perfilLabel={meuMedico?.especialidade ?? "Médico(a)"}
        titulo={`${usuarioLogado.nome}`}
        descricao="Acompanhe e atenda os pacientes agendados com você. Atualize o status de cada atendimento conforme avança a consulta."
      />
        ...
        <section aria-label="Resumo do médico" className="grid grid-cols-3 gap-4">
          <article className="rounded-2xl border-2 border-accent/30 bg-accent/5 p-5">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-widest text-accent font-bold">Aguardando</p>
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-accent text-accent-foreground">
                <Clock size={18} aria-hidden="true" />
              </span>
            </div>
            <p className="font-display text-4xl font-bold mt-3 text-accent">{aguardando.length}</p>
          </article>
          <article className="rounded-2xl border-2 border-primary/30 bg-primary/5 p-5">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-widest text-primary font-bold">Em atendimento</p>
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-primary text-primary-foreground">
                <Stethoscope size={18} aria-hidden="true" />
              </span>
            </div>
            <p className="font-display text-4xl font-bold mt-3 text-primary">{ematendimento.length}</p>
          </article>
          <article className="rounded-2xl border-2 border-border bg-card p-5">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold">Encerrados</p>
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-secondary text-secondary-foreground">
                <CheckCircle2 size={18} aria-hidden="true" />
              </span>
            </div>
            <p className="font-display text-4xl font-bold mt-3">{encerrados.length}</p>
          </article>
        </section>

        {/* Próximo paciente em destaque */}
        {(ematendimento[0] || aguardando[0]) && (
          <section aria-labelledby="proximo-paciente">
            <h2 id="proximo-paciente" className="font-display text-2xl font-bold mb-4">
              {ematendimento[0] ? "Atendendo agora" : "Próximo paciente"}
            </h2>
            {(() => {
              const ag = ematendimento[0] ?? aguardando[0]
              const data = formatarDataHora(ag.dataHora)
              return (
                <article
                ...
                >
                  <div className="grid lg:grid-cols-[auto,1fr,auto] gap-6 items-start">
                    ...
                    >
                      <p className="text-xs uppercase tracking-widest font-bold opacity-90">Hoje</p>
                      <p className="font-display text-3xl font-bold mt-1">{data.hora}</p>
                    </div>
                        ...
                        <div>
                          <p
                            className={`text-xs uppercase tracking-widest font-bold ${
                              ematendimento[0] ? "text-primary-foreground/70" : "text-muted-foreground"
                            }`}
                          >
                            Paciente
                          </p>
                          ...
                          <p className={`text-sm mt-1 ${ematendimento[0] ? "text-primary-foreground/85" : "text-muted-foreground"}`}>
                            Mãe: {ag.pacienteNome} · {ag.tipoDeficiencia}
                          </p>
                        </div>
                      ...
                      {ag.observacoes && (
                        <div
                          className={`mt-5 rounded-xl p-4 ${
                            ematendimento[0] ? "bg-primary-foreground/10" : "bg-card border border-border"
                          }`}
                        >
                          ...
                          <p className="text-sm leading-relaxed">{ag.observacoes}</p>
                        </div>
                      )}
                    </div>
        ...
        <section aria-labelledby="agenda">
          ...
          <Tabs defaultValue="aguardando">
            <TabsList className="grid grid-cols-3 w-full max-w-2xl">
              ...
              <TabsTrigger value="ematendimento" className="gap-2">
            ...
            <TabsContent value="aguardando" className="mt-4 space-y-4">
              {aguardando.length === 0 ? (
                <EmptyState Icon={ClipboardList} mensagem="Nenhum paciente aguardando." />
              ) : (
                aguardando.map((a) => (
                  <AppointmentCard
                    key={a.id}
            ...
            <TabsContent value="ematendimento" className="mt-4 space-y-4">
              {ematendimento.length === 0 ? (
                <EmptyState Icon={Stethoscope} mensagem="Nenhum atendimento em curso." />
              ) : (
                ematendimento.map((a) => (
                  <AppointmentCard
                    key={a.id}
            ...
            <TabsContent value="encerrados" className="mt-4 space-y-4">
              {encerrados.length === 0 ? (
                <EmptyState Icon={Users} mensagem="Nenhum atendimento encerrado ainda." />
              ) : (
                encerrados.map((a) => <AppointmentCard key={a.id} agendamento={a} />)
              )}
            </TabsContent>
          </Tabs>
        </section>
      ...
      <Dialog open={encerrarOpen} onOpenChange={setEncerrarOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-display text-2xl">Encerrar atendimento</DialogTitle>
            <DialogDescription>
              {agSelecionado &&
                `Confirme o encerramento da consulta de ${agSelecionado.criancaNome}. O status mudará para "atendimento encerrado".`}
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-3">
            <Label htmlFor="enc-obs" className="text-sm font-semibold">
              Observações finais <span className="font-normal text-muted-foreground">(opcional)</span>
            </Label>
            <Textarea
              id="enc-obs"
